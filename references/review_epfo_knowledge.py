...
LIST_FIELDS = (
    "aliases", "claim_types_affected", "fix_steps", "required_documents",
...
def build_inventory(records: list[dict], corpus: Path = OUTPUT) -> dict:
    ...
    for record in records:
        ...
        rows.append({
            ...
            "empty_fields": [field for field, value in record.items() if value == "" or value == []],
            ...
            "review_signals": [
                term for term in ("2026", "verify", "secondary", "conflict", "not independently")
                ...
                and (term != "2026" or "2026" in " ".join(
                    str(value) for field, value in record.items() if field != "last_verified"
                ))
            ],
        })
    fingerprints = {}
    for path in sorted([ARCHIVE / "data/rejections.json", ARCHIVE / "data/source_links.json", ARCHIVE / "sources.md", *sorted((ARCHIVE / "docs").glob("*.md")), *sorted(corpus.rglob("*.md"))]):
        label = str(path.relative_to(ROOT)).replace("\\", "/") if path.is_relative_to(ROOT) else "corpus/" + path.relative_to(corpus).as_posix()
    ...
    for url, entry in catalog_by_url.items():
        ...
        if sorted(entry["used_by_reason_ids"]) != expected_ids or entry["use_count"] != len(expected_ids):
    ...
    return {
        ...
        "shared_normalized_aliases": {alias: sorted(ids) for alias, ids in sorted(aliases.items()) if len(ids) > 1},
        "index_sections": {heading: {"bytes": len(body.encode("utf-8")), "lines": len(body.splitlines())} for heading, body in index.items()},
        "records": rows,
...
def validate_review_cases(cases: list[dict], records: list[dict], corpus: Path = OUTPUT) -> None:
    ...
    for case in cases:
        ...
        if not set(case["expected_states"]) <= {"success", "needs_clarification", "unsupported"} or not case["expected_states"]:
...
def main() -> None:
    ...
    serialized = json.dumps(build_inventory(records), ensure_ascii=False, indent=2) + "\n"
    ...
    print(f"Checked {len(records)} readable records, {len(cases)} review cases, {len(register)} verification gaps. No live sources or agent execution verified.")
