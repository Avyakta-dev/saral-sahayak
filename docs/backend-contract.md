...
- **Cleanup limits:** each admitted request attempts deletion of inbox and validated objects on success, failure and cancellation, with two bounded SDK delete attempts. Lifecycle covers failed deletes, crashes, expired/unconsumed tickets and late PUT replay. Signed PUT URLs cannot be revoked by consuming a ticket or deleting an object: they can recreate the inbox object until signature expiry, but cannot authorize another analysis. URL expiry does **not** imply object deletion or provider cache/retention expiry. Do not promise immediate erasure.
