...
Main now includes `docs/protected-analysis.md`: public deployments require a separately authenticated gateway; the server-to-server access token must never be put into the extension, frontend settings or CORS. This extension's fixed loopback development destination has no such browser-authenticated gateway integration. Protected 401/429/outer-504 small envelopes must remain honest errors; never copy the server token into browser code to make a demo pass. The initial generic unexpected-HTTP fallback was subsequently replaced with explicit safe handling for small-envelope HTTP 401/403, 429 and 504. Access-denial messages state that shared server credentials must not enter the browser. Capacity messages display only an integer Retry-After from 1 to 3,600 seconds, otherwise omit it; no retry is scheduled. Server-provided message text is never echoed through this small-envelope path. Existing validated full analysis envelopes retain their own error handling. No endpoint, authentication setting, permission or CORS change was made. This is transport/UI error handling, not authenticated-gateway implementation or full deployment acceptance.
