...
Use Chrome Extensions → Developer mode → Load unpacked → select `extension` → Extensions menu → Saral Sahayak. Do not use a normal tab at `popup.html` as a replacement for its toolbar action. Enter the synthetic strings above, press Tab, and use Clear session. Native Chrome paste and keyboard Clear are now evidenced above; complete keyboard/assistive checks and Brave still require evidence. Reviewer decides issue acceptance; this report does not close the issue automatically.
