...
A later [actual Chrome check](../evidence/privacy-17-browser.md) verified the trusted local window, explicit metadata inspection and fail-closed UI on worker disconnect. Capture was not completed because the desktop helper refused the input action; pixel rendering and post-capture lifecycle remain unverified. Complete browser acceptance, selective pixel redaction, Avyakta UI integration and exhaustive cross-mode privacy evidence are **not complete**. Keep issue 17 open pending review of this limited slice and the remaining acceptance scope. Image transport still depends on the separately approved controlled host/operator and image implementation.
